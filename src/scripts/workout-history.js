...
import { showFilterOptions, hideFilterOptions } from "../modules/filter.mjs";
import { updateUserStats, updateUserExercises } from "../modules/stats.mjs";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let filterOptionVisible = { value: false };
  let filterOptionContainer = { value: null };
  ...
  renderWorkoutHistory({ workouts, users, currentUser });

  filterButton.addEventListener("click", () => {
    if (filterOptionVisible.value) {
      hideFilterOptions({
        filterOptionVisible,
