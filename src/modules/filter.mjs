...
export function showFilterOptions({
  ...
  filterOptionVisible.value = true;

  if (!filterOptionContainer.value) {
    // Create filterOptionContainer
    const container = document.createElement("div");
    container.classList.add("filter-options-container");
    ...
    applyFilterButton.addEventListener("click", () =>
      applyFilters({ workouts, renderWorkoutHistory })
    );
    ...
    filterOptionContainer.value = container;
  }

  // Append filterOptionContainer to desired location (e.g., below header)
  const header = document.querySelector("header");
  header.insertAdjacentElement("afterend", filterOptionContainer.value);
}

export function hideFilterOptions({
  filterOptionVisible,
  filterOptionContainer,
  renderWorkoutHistory,
  workouts,
}) {
  filterOptionVisible.value = false;
  if (filterOptionContainer.value && filterOptionContainer.value.parentNode) {
    filterOptionContainer.value.parentNode.removeChild(
      filterOptionContainer.value
    );
  }
...
function applyFilters({ workouts, renderWorkoutHistory }) {
  ...
  renderWorkoutHistory({ workouts: filteredWorkouts });
}
